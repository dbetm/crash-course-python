...
"""Because we used the @value decorator, Grid includes a "member-wise" constructor.
The constructor's arguments are the same names and types as the struct's fields 
and appear in the same order."""
@value # Mojo compiler automatically generates lifecycle methods for you.
struct Grid():
    var rows: Int
    var cols: Int
    ...
    # in order to use standar [] for indexing
    def __getitem__(self, row: Int, col: Int) -> Int:
        return self.data[row][col]

    def __setitem__(mut self, row: Int, col: Int, value: Int) -> None:
        """we've added the mut argument convention to the self argument to explicitly tell the Mojo 
        compiler that we want to mutate the state of the current instance. If we were to omit mut, we would
        get an error because the compiler would default to read-only access for the argument.
        """
